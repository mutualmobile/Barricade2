...
class BarricadeProcessor(
    ...
    override fun process(resolver: Resolver): List<KSAnnotated> {
        ...
        file writeToFile "\t\tval barricadeResponsesForRandom = mutableListOf<com.mutualmobile.barricade2.response.BarricadeResponse>()\n"
        symbols.forEach { symbol ->
            symbol.accept(BarricadeVisitor(file, logger), Unit)
        }
        file writeToFile "\t\tconfigs.put(\"random\", com.mutualmobile.barricade2.response.BarricadeResponseSet(barricadeResponsesForRandom, 0))\n"
        file writeToFile "\t}\n\n"
